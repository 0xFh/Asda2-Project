...
namespace WCell.Util.Threading.TaskParallel
{
    ...
    public static class BlockingCollectionExtensions
    {
            ...
            bool completeAddingWhenDone)
        {
            ...
            return source.Subscribe((IObserver<T>) new DelegateBasedObserver<T>((Action<T>) (item => target.Add(item)),
        ...
        {
            return (IProducerConsumerCollection<T>) new BlockingCollectionExtensions.ProducerConsumerWrapper<T>(
                collection, millisecondsTimeout, new CancellationToken());
        }
        ...
        private class BlockingCollectionPartitioner<T> : Partitioner<T>
        {
            ...
            public override IList<IEnumerator<T>> GetPartitions(int partitionCount)
            {
                ...
                return (IList<IEnumerator<T>>) Enumerable.Range(0, partitionCount)
                    .Select<int, IEnumerator<T>>((Func<int, IEnumerator<T>>) (_ => dynamicPartitioner.GetEnumerator()))
                    .ToArray<IEnumerator<T>>();
            }
        ...
        /// <typeparam name="T">Specifies the type of the elements in the collection.</typeparam>
        internal sealed class ProducerConsumerWrapper<T> : IProducerConsumerCollection<T>, IEnumerable<T>, ICollection,
            IEnumerable
        {
            ...
            public ProducerConsumerWrapper(BlockingCollection<T> collection, int millisecondsTimeout,
            ...
            {
                get { return ((ICollection) this._collection).IsSynchronized; }
            }

            public object SyncRoot
            {
                get { return ((ICollection) this._collection).SyncRoot; }
            }
        }
    }
}