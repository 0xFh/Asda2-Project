...
namespace WCell.RealmServer.AI.Brains
{
	...
	public class BaseBrain : IBrain
	{
		...
		public virtual bool CheckCombat()
		{
			...
			if (!m_owner.CanDoHarm || m_owner.IsInfluenced)
			{
				return false;
			}
			...
			if ((owner.ThreatCollection.CurrentAggressor != null && owner.CanReachForCombat(owner.ThreatCollection.CurrentAggressor)) ||
				 (m_IsAggressive && TryScanAndAttack()))
			{
	    ...
	    private int _updatesCountFrorScanAndAttack = 0;
        
        /// <summary>
        /// scan only every 50 update
        /// </summary>
        /// <returns></returns>
	    private bool TryScanAndAttack()
	    {
            if (_updatesCountFrorScanAndAttack >= CharacterFormulas.NpcUpdatesToScanAndAttack)
            {
                _updatesCountFrorScanAndAttack = 0;
               return ScanAndAttack();
            }
            _updatesCountFrorScanAndAttack++;
            return false;
	    }
