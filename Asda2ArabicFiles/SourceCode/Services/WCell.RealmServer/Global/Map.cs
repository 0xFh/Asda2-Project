...
namespace WCell.RealmServer.Global
{
    ...
    public partial class Map : IGenericChatTarget, IMapId, IContextHandler, IWorldSpace
    {
        ...
        public void ClearLootOnGuildWaveNow(int guildid)
        {
            ...
            {
                Asda2Loot asda2Loot = Loots[i];

                foreach (var asda2LootItem in asda2Loot.Items)
                {
                    GlobalHandler.SendRemoveItemOnGuildWaveResponse(asda2LootItem, guildid);
                    ClearLootSlot(asda2LootItem.Position.X, asda2LootItem.Position.Y);
                }
            ...
            else
            {
                ...
                if (oldMap != null)
                {
                    var moveTask = new Message(() =>
                    {
                        ...
                        if (chr != null)
                        {
                            GlobalHandler.SendTeleportedByCristalResponse(chr.Client, MapTemplate.RealId.Value, (short)(newPos.X - Offset), (short)(newPos.Y - Offset));
