...
namespace WCell.RealmServer.NPCs
{
	...
	public static class NPCMgr
	{
		...
		/// </summary>
		public static float DefaultMaxHomeDistanceInCombatSq = 900f;
		...
		[NotVariable]
		internal static Spell[][] PetSpells = new Spell[][]{};
		#endregion
		...
		{
            DefaultFaction = new Faction(){Id = FactionId.None};
            DefaultFamily = new CreatureFamily(){Id = CreatureFamilyId.None};
			InitDefault();
...
//#endif
		...
		{
			/*var npcSpells = new MappedDBCReader<Spell[], DBCCreatureSpellConverter>(RealmServerConfiguration.GetDBCFile(WCellConstants.DBC_CREATURESPELLDATA)).Entries;
		...
		public static void BuySpell(this NPC trainer, Character chr, SpellId spellEntryId)
		{
			...
			chr.SubtractMoney(trainerSpell.GetDiscountedCost(chr, trainer));
