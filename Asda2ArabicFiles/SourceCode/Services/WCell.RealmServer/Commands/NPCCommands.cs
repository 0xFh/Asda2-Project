    ...
    public class NPCCommand : RealmServerCommand
	{
		...
		public class AddNPCCommand : SubCommand
		{
			...
			public override void Process(CmdTrigger<RealmServerCmdArgs> trigger)
			{
                ...
                {
                    //remove
                    var target = trigger.Args.Target as NPC;
                    if(target == null)
                    {
                    ...
                    var entry =target.SpawnEntry;
                    entry.CommitDeleteAndFlush();
                    trigger.Reply("Monstr {0} deleted from spawn.",entry);
                    return;
                }
