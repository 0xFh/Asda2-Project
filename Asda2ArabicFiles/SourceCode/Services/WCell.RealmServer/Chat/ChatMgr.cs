...
/*
		...
		{
		    using (var p =CreateGlobalChatMessage("«·”Ì—›—",message,Color.DodgerBlue, target.Locale))
		    {
        ...
        public static void SendSystemMessage(this IEnumerable<Character> targets, string message)
		{
		    ...
		    foreach (var target in targets)
		    {
		        ...
		        using (
		            var packet = CreateGlobalChatMessage("«·”Ì—›—", message, Color.DodgerBlue, target.Client.Locale)
		            )
		        {
