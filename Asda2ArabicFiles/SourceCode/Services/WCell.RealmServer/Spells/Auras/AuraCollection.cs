...
namespace WCell.RealmServer.Spells.Auras
{
	...
	public class AuraCollection : IEnumerable<Aura>
	{
			...
			try
			{
				...
				if (oldAura != null)
				{
					if (!PrepareStackOrOverride(caster, oldAura, spell, ref err))
					{
						...
						{
							SpellHandler.SendCastFailed((Character)caster.Object, spell, err);
						}
						return null;
					}
				}
		...
		public virtual void AddAura(Aura aura, bool start = true)
		{
            ...
            foreach(var a in m_auras.Values){
                if (a.Spell.RealId == aura.Spell.RealId)
                {
                    a.Remove();
                    break;
                }
			...
			var bonus = 0;
			{
				foreach (var aura in m_AuraArray)
				{
					foreach (var handler in aura.Handlers)
					{
						...
						{
							bonus += handler.SpellEffect.MiscValue;
						}
					}
				}
			}
		...
		internal void SaveAurasNow()
		{
			foreach (var aura in m_visibleAuras)
			{
				if (aura != null && aura.CanBeSaved && (!aura.HasTimeout || aura.TimeLeft > 5000))
				{
                    ...
                    {
                        LogUtil.WarnException(ex, string.Format("failed to save aura, character {0} acc {1}[{2}]", OwnerChar.Name, OwnerChar.Account.Name, OwnerChar.AccId));
                    }
				}
			}
		}
