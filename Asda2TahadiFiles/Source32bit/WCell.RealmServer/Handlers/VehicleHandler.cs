  ...
  public static class VehicleHandler
  {
    ...
    public static void HandleChangeSeatsOnControlledVehicle(IRealmClient client, RealmPacketIn packet)
    {
      ...
      if(vehicle1 == null)
        return;
      uint clientTime;
      MovementHandler.ReadMovementInfo(packet, client.ActiveCharacter, vehicle1, out clientTime);
      ...
      if(vehicleSeat == null)
        vehicle1.FindSeatOccupiedBy(activeCharacter);
      if(vehicleSeat == null)
        return;
      byte num = packet.ReadByte();
      VehicleSeat seat = vehicle2.Seats[num];
      if(seat == null || seat.Passenger != null)
        return;
      vehicleSeat.ClearSeat();
      seat.Enter(activeCharacter);
    }

    public static void SendBreakTarget(IPacketReceiver rcvr, IEntity target)
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_BREAK_TARGET, 8))
      {
    ...
    public static void Send_SMSG_ON_CANCEL_EXPECTED_RIDE_VEHICLE_AURA(Character chr)
    {
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_ON_CANCEL_EXPECTED_RIDE_VEHICLE_AURA, 0))
