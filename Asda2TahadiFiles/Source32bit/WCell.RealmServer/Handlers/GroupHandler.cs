  ...
  public static class GroupHandler
  {
    ...
    public static void ConvertToRaidRequest(IRealmClient client, RealmPacketIn packet)
    {
      ...
      if(!(group1 is PartyGroup) || !group1.CheckPrivs(groupMember, GroupPrivs.Leader))
        return;
      Group group2 = ((PartyGroup) group1).ConvertTo();
      SendResult(client, GroupResult.NoError);
      group2.SendUpdate();
    }
    ...
    public static void SendGroupInvite(IPacketReceiver client, string inviter)
    {
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_GROUP_INVITE, inviter.Length + 10))
      {
        packet.Write((byte) 1);
    ...
    {
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_GROUP_DECLINE, decliner.Length + 1))
      {
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.MSG_RANDOM_ROLL, 20))
      {
        packet.Write(min);
        packet.Write(max);
        packet.Write(value);
    ...
    {
      SendPartyMemberStatsInternal(client, member, flags,
