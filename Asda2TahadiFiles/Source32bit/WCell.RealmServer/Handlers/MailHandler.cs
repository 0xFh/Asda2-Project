...
namespace WCell.RealmServer.Handlers
{
  public static class MailHandler
  {
    ...
    public static void HandleListMail(IRealmClient client, RealmPacketIn packet)
    {
      ...
      if(!CheckMailBox(activeCharacter, activeCharacter.Map.GetObject(id) as GameObject))
        return;
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_SEND_MAIL_RESULT, 12))
      {
        packet.Write(mailId);
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_SEND_MAIL_RESULT, 16))
      {
        packet.Write(mailId);
        packet.Write((uint) result);
        packet.Write((uint) err);
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_MAIL_LIST_RESULT,
        128 * messages.Count))
      {
        packet.Write(messages.Count);
        int num1 = Math.Min(messages.Count, byte.MaxValue);
          ...
          if(!message.IsDeleted)
          {
            ...
            switch(message.MessageType)
            {
              case MailType.Normal:
                packet.Write(message.SenderEntityId);
                break;
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_RECEIVED_MAIL, 4))
      {
        packet.Write(0);
        client.Send(packet, false);
      }
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.MSG_QUERY_NEXT_MAIL_TIME, 8))
      {
        if(mail.Count <= 0)
        {
        ...
        else
        {
          ...
          foreach(MailMessage mailMessage in mail)
          {
            packet.Write(mailMessage.SenderEntityId);
