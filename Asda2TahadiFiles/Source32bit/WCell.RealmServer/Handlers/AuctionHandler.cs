...
namespace WCell.RealmServer.Handlers
{
  public static class AuctionHandler
  {
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.MSG_AUCTION_HELLO, 12))
      {
      ...
      uint minBidInc)
    {
      if(auction == null)
        return;
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_AUCTION_BIDDER_NOTIFICATION, 32))
      {
    ...
    public static void SendAuctionListOwnerItems(IPacketReceiver client, Auction[] auctions)
    {
      ...
      RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_AUCTION_OWNER_LIST_RESULT, 1024);
    ...
    public static void SendAuctionListBidderItems(IPacketReceiver client, Auction[] auctions)
    {
      if(auctions == null || auctions.Length < 1)
        return;
      RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_AUCTION_BIDDER_LIST_RESULT, 1024);
      packet.Write(auctions.Length);
      foreach(Auction auction in auctions)
        BuildAuctionPacket(auction, packet);
      packet.Write(auctions.Length);
