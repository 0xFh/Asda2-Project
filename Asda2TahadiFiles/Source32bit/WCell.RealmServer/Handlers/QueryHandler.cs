...
namespace WCell.RealmServer.Handlers
{
  public static class QueryHandler
  {
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_QUERY_TIME_RESPONSE, 4))
      {
      ...
      if(entity != null)
        ...
        ServerApp<RealmServer>.IOQueue.AddMessage(new Message(() =>
        {
          ...
          if(characterRecord == null)
            s_log.Warn("{0} queried name of non-existing Character: " + id,
              client);
          else
    ...
    public static void HandleNPCTextQuery(IRealmClient client, RealmPacketIn packet)
    {
      ...
      if(entry == null)
        return;
      SendNPCTextUpdate(client.ActiveCharacter, entry);
    }
    ...
    public static void SendPageText(Character chr, uint pageId)
    {
      ...
      else
      {
        using(RealmPacketOut packet =
          new RealmPacketOut(RealmServerOpCode.SMSG_PAGE_TEXT_QUERY_RESPONSE, 100))
        {
