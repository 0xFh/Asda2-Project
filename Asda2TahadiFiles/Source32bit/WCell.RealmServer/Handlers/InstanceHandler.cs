  ...
  public class InstanceHandler
  {
    ...
    public static void SendInstanceReset(IPacketReceiver client, MapId mapId)
    {
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_UPDATE_LAST_INSTANCE, 4))
      {
    ...
    {
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_INSTANCE_RESET_FAILED, 8))
      {
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_RESET_FAILED_NOTIFY, 4))
      {
        packet.Write((uint) map);
        client.Send(packet, false);
      }
    ...
    public static void SendRaidDifficulty(Character chr)
    {
      using(RealmPacketOut realmPacketOut =
        new RealmPacketOut(RealmServerOpCode.MSG_SET_RAID_DIFFICULTY, 12))
      {
