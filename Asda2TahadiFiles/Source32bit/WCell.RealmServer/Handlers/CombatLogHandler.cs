...
namespace WCell.RealmServer.Handlers
{
  public static class CombatLogHandler
  {
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_SPELLLOGMISS, 34))
      {
      ...
      int spellId, bool b1)
    {
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_SPELLORDAMAGE_IMMUNE, 21))
      {
    ...
    public static void SendEnergizeLog(WorldObject caster, Unit target, uint spellId, PowerType powerType,
      int value)
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_SPELLENERGIZELOG, 25))
      {
        target.EntityId.WritePacked(packet);
        caster.EntityId.WritePacked(packet);
        packet.Write(spellId);
    ...
    {
      RealmPacketOut realmPacketOut = new RealmPacketOut(RealmServerOpCode.SMSG_SPELLLOGEXECUTE, 37);
      caster.EntityId.WritePacked(realmPacketOut);
        ...
        for(int index2 = 0; index2 < 1; ++index2)
        {
          switch(effect)
          {
            case SpellEffectType.PowerDrain:
              target.EntityId.WritePacked(realmPacketOut);
    ...
    public static void SendHealLog(WorldObject caster, Unit target, uint spellId, int value, bool critical,
      int overheal)
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_SPELLHEALLOG, 25))
      {
        target.EntityId.WritePacked(packet);
        caster.EntityId.WritePacked(packet);
        packet.Write(spellId);
    ...
    {
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_ENVIRONMENTALDAMAGELOG, 21))
      {
        target.EntityId.WritePacked(packet);
    ...
    {
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_SPELLNONMELEEDAMAGELOG, 40))
      {
