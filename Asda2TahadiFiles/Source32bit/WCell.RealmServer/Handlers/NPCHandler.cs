  ...
  public static class NPCHandler
  {
    ...
    public static void SendBankSlotResult(Character chr, BuyBankBagResponse response)
    {
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_BUY_BANK_SLOT_RESULT, 4))
      {
    ...
    public static void HandlePetitionerShowList(IRealmClient client, RealmPacketIn packet)
    {
      ...
      if(petitioner == null || !petitioner.NPCFlags.HasFlag(NPCFlags.Petitioner))
        return;
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_PETITION_SHOWLIST, 32))
      {
    ...
    public static void HandlePetitionDecline(IRealmClient client, RealmPacketIn packet)
    {
      ...
      SendPetitionDecline(client, client.ActiveCharacter, record);
    }
    ...
    public static void SendPetitionSignatures(IPacketReceiver client, PetitionCharter charter)
    {
      ...
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_PETITION_SHOW_SIGNATURES,
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.MSG_PETITION_DECLINE, 8))
      {
        Character character = Global.World.GetCharacter(record.OwnerId);
        if(character == null)
          return;
    ...
    {
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_TURN_IN_PETITION_RESULTS, 4))
      {
    ...
    public static void SendPetitionRename(IPacketReceiver client, PetitionCharter petition)
    {
      ...
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.MSG_PETITION_RENAME, 8 + name.Length + 1))
      {
    ...
    public static void SendPetitionQueryResponse(IPacketReceiver client, PetitionCharter charter)
    {
      ...
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_PETITION_QUERY_RESPONSE,
    ...
    public static void SendNPCError(IPacketReceiver client, IEntity vendor, VendorInventoryError error)
    {
      using(RealmPacketOut realmPacketOut =
        new RealmPacketOut(RealmServerOpCode.SMSG_LIST_INVENTORY, 10))
      {
        realmPacketOut.Write(vendor.EntityId);
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_LIST_INVENTORY,
        10 + 28 * itemsForSale.Count()))
      {
        packet.Write(vendor.EntityId);
        long position = packet.Position;
        packet.WriteByte(0);
        int val = 0;
        foreach(VendorItemEntry vendorItemEntry in itemsForSale.Where(
          item => item != null))
        {
          if(buyer.GodMode ||
    ...
    public static void SendSellError(IPacketReceiver client, EntityId vendorId, EntityId itemId,
      SellItemError error)
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_SELL_ITEM, 17))
      {
    ...
    public static void SendBuyError(IPacketReceiver client, IEntity vendor, Asda2ItemId itemEntryId,
      BuyItemError error)
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_BUY_FAILED, 13))
      {
        packet.Write(vendor.EntityId);
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_BUY_ITEM, 20))
      {
        packet.Write(vendor.EntityId);
      ...
      TrainerBuyError error)
    {
      ...
      {
        realmPacketOut.Write(trainer.EntityId);
    ...
    {
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_TRAINER_LIST, 1156 + msg.Length + 1))
      {
    ...
    {
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_TRAINER_BUY_SUCCEEDED, 12))
      {
        packet.Write(trainer.EntityId);
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_BINDER_CONFIRM, 12))
      {
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_PLAYERBOUND, 12))
      {
        packet.Write(binder.EntityId);
        packet.Write((uint) zone);
        chr.Client.Send(packet, false);
