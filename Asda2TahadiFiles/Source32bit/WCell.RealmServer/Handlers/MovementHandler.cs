...
namespace WCell.RealmServer.Handlers
{
  public static class MovementHandler
  {
    ...
    public static void HandleMovement(IRealmClient client, RealmPacketIn packet)
    {
      ...
      uint clientTime;
      if(!ReadMovementInfo(packet, activeCharacter, mover, out clientTime))
        return;
      BroadcastMovementInfo(packet, activeCharacter, mover, clientTime);
    }
    ...
    public static bool ReadMovementInfo(RealmPacketIn packet, Character chr, Unit mover, out uint clientTime)
    {
      ...
      {
        if(movementFlags.HasFlag(MovementFlags.Flying) && !chr.CanFly)
          return false;
    ...
    {
      ICollection<IRealmClient> nearbyClients = chr.GetNearbyClients(false);
      if(nearbyClients.Count <= 0)
        return;
      using(RealmPacketOut pak = new RealmPacketOut(packet.PacketId))
      {
        int num = mover.EntityId.WritePacked(pak);
        packet.Position = packet.HeaderSize + num;
        pak.Write(packet.ReadBytes(packet.RemainingLength));
        foreach(IRealmClient client in nearbyClients)
          SendMovementPacket(client, pak, 10 + num, clientTime);
      }
    }
    ...
    public static void SendEnterTransport(Unit unit)
    {
      ...
      SendMonsterMoveTransport(unit, transportInfo.Position - unit.Position,
    ...
    public static void SendMonsterMoveTransport(Unit unit, Vector3 from, Vector3 to)
    {
      ...
      {
        ITransportInfo transportInfo = unit.TransportInfo;
        unit.EntityId.WritePacked(realmPacketOut);
        transportInfo.EntityId.WritePacked(realmPacketOut);
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_MOUNTSPECIAL_ANIM, 8))
      {
      ...
      IEnumerable<T> waypoints) where T : IPathVertex
    {
      ...
      using(RealmPacketOut packet =
        ConstructMultiWaypointMovePacket(movingUnit, moveTime, moveFlags, waypoints))
      ...
      LinkedListNode<T> firstNode) where T : IPathVertex
    {
      if(!movingUnit.IsAreaActive)
        return;
      using(RealmPacketOut packet =
        ConstructMultiWaypointMovePacket(movingUnit, speed, moveFlags, firstNode))
        movingUnit.SendPacketToArea(packet, true, false, Locale.Any, new float?());
    }
    ...
    public static RealmPacketOut ConstructMultiWaypointMovePacket<T>(Unit movingUnit, uint moveTime,
      MonsterMoveFlags moveFlags, IEnumerable<T> waypoints) where T : IPathVertex
    {
      int num = waypoints.Count();
      RealmPacketOut writer =
        new RealmPacketOut(RealmServerOpCode.SMSG_MONSTER_MOVE, 38 + num * 4 * 3);
      movingUnit.EntityId.WritePacked(writer);
      writer.Write(false);
                              ...
                              MonsterMoveFlags.Flag_0x40000_FullPoints_2))
      {
        ...
        {
          IPathVertex pathVertex = waypoint;
      ...
      else
      {
        ...
        {
          IPathVertex pathVertex = waypoint;
          writer.Write(pathVertex.Position.ToDeltaPacked(movingUnit.Position, waypoints.First().Position));
        }
      }
    ...
    {
      RealmPacketOut writer = new RealmPacketOut(RealmServerOpCode.SMSG_MONSTER_MOVE,
        39 + firstNode.List.Count * 4 * 3);
      unit.EntityId.WritePacked(writer);
      writer.Write(false);
      ...
      writer.Position += 4L;
      int num1 = (int) (1000.0 * unit.Position.GetDistance(firstNode.Value.Position) / speed);
      int num2 = 0;
      ...
      while(true)
      {
        ...
        if(next != null)
        {
          num1 += (int) (1000.0 * linkedListNode.Value.GetDistanceToNext() / speed);
          linkedListNode = next;
        }
        else
          break;
