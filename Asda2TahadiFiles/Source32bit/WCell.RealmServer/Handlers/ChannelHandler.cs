...
namespace WCell.RealmServer.Handlers
{
  public static class ChannelHandler
  {
    ...
    public static void HandleListChannel(IRealmClient client, RealmPacketIn packet)
    {
      ...
      ChatChannel chan = ChatChannelGroup.RetrieveChannel(client.ActiveCharacter, str);
      if(chan != null)
        SendChannelList(client, chan);
      else
        SendNotOnChannelReply(client, str);
    }
    ...
    public static void HandlePasswordChange(IRealmClient client, RealmPacketIn packet)
    {
      ...
      ChatChannel chan = ChatChannel.EnsurePresence(client.ActiveCharacter, str1, out member);
      if(chan != null)
      {
        if(!member.IsModerator)
        {
          SendNotModeratorReply(client, str1);
        }
        else
        {
          chan.Password = str2;
        ...
        SendNotOnChannelReply(client, str1);
    }
    ...
    {
      string channelName = packet.ReadCString();
      ChatChannel chan = ChatChannel.EnsureModerator(client.ActiveCharacter, channelName);
      if(chan == null)
        return;
    ...
    {
      foreach(ChannelMember channelMember in chan.Members.Values)
