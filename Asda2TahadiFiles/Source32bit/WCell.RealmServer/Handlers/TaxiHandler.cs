...
namespace WCell.RealmServer.Handlers
{
  public static class TaxiHandler
  {
    ...
    public static void HandleTaxiStatusQuery(IRealmClient client, RealmPacketIn packet)
    {
      ...
      SendTaxiNodeStatus(client, npc.VendorTaxiNode, entityId, isActiveNode);
    }
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_NEW_TAXI_PATH, 0))
        client.Send(packet, false);
    }

    public static void SendTaxiPathUpdate(IPacketReceiver client, EntityId vendorId, bool activated)
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_TAXINODE_STATUS, 9))
      {
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_SHOWTAXINODES, 48))
      {
        packet.Write(1);
        if(vendor != null)
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_ACTIVATETAXIREPLY, 4))
      {
