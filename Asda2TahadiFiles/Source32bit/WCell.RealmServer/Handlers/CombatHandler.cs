  ...
  public static class CombatHandler
  {
    ...
    public static void SendCombatStop(Unit attacker, Unit opponent, int extraArg)
    {
      if(!attacker.IsAreaActive)
        return;
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_ATTACKSTOP, 22))
      {
    ...
    public static void SendAttackerStateUpdate(DamageAction action)
    {
      ...
      using(RealmPacketOut packet =
        new RealmPacketOut(RealmServerOpCode.SMSG_ATTACKERSTATEUPDATE, 100))
      {
    ...
    {
      using(RealmPacketOut packet = new RealmPacketOut(RealmServerOpCode.SMSG_UPDATE_COMBO_POINTS, 9))
      {
