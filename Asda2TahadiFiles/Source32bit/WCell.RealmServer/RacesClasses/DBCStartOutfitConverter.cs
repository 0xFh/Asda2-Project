...
namespace WCell.RealmServer.RacesClasses
{
  public class DBCStartOutfitConverter : DBCRecordConverter
  {
    public override void Convert(byte[] rawData)
    {
      ...
      RaceId race = (RaceId) ((int) uint32 & byte.MaxValue);
      ...
      {
        int itemId = GetInt32(rawData, field);
        if(itemId > 0)
        {
          ItemTemplate template = ItemMgr.GetTemplate((Asda2ItemId) itemId);
          if(template == null)
            LogManager.GetCurrentClassLogger()
              .Warn("Missing initial Item in DB: " + itemId + " (" + (uint) itemId +
                    ")");
          else if(template.IsStackable)
          {
            int index = initialItems.FindIndex(stack =>
