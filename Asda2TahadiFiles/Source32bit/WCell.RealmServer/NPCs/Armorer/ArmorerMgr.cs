...
namespace WCell.RealmServer.NPCs.Armorer
{
  public static class ArmorerMgr
  {
    ...
    {
      if(item != null)
        return ArmorerCheatChecks(curChar, armorer);
      return false;
    }
    ...
    private static uint GetCostToRepair(Item item)
    {
      ...
      if(num1 <= 0U)
        return 0;
      DurabilityCost classRepairModifier = itemClassRepairModifiers[item.Template.Level];
      ...
      DurabilityQuality qualityRepairModifier =
        itemQualityRepairModifiers[(int) (item.Template.Quality + 1U) * 2];
