...
namespace WCell.RealmServer.Entities
{
  public class GameTables
  {
    ...
    {
      return ModifyValue(GetValuePerRating(ClassSpellCritChance, level, id));
    }
    ...
    {
      return ModifyValue(GetValuePerRating(ClassMeleeCritChance, level, id));
    }
    ...
    {
      return LoadRatingChanceDBC("gtChanceToSpellCritBase.dbc", out s_baseSpellCritChance);
    }

    private static bool LoadGtClassSpellCritChanceDBC()
    {
      return LoadRatingChanceDBC("gtChanceToSpellCrit.dbc", out s_classSpellCritChance);
    }

    private static bool LoadGtBaseMeleeCritChanceDBC()
    {
      return LoadRatingChanceDBC("gtChanceToMeleeCritBase.dbc", out s_baseMeleeCritChance);
    }

    private static bool LoadGtClassMeleeCritChanceDBC()
    {
      return LoadRatingChanceDBC("gtChanceToMeleeCrit.dbc", out s_classMeleeCritChance);
    }

    private static bool LoadGtClassHealthRegenPerSpiritDBC()
    {
      return LoadRatingChanceDBC("GtRegenHPPerSpt.dbc", out s_octHealthRegenPerSpirit);
    }

    private static bool LoadGtClassManaRegenPerSpiritDBC()
    {
      return LoadRatingChanceDBC("GtRegenMPPerSpt.dbc", out s_octManaRegenPerSpirit);
    }

    private static bool LoadGtClassOCTHealthRegenDBC()
    {
      return LoadRatingChanceDBC("GtOCTRegenHP.dbc", out s_octHealthRegen);
    }

    private static bool LoadGtClassOCTManaRegenDBC()
    {
      return LoadRatingChanceDBC("GtOCTRegenMP.dbc", out s_octManaRegen);
    }
    ...
    public static bool LoadGtDBCs()
    {
      ...
      {
        s_log.Info(string.Format(WCell_RealmServer.DBCLoadFailed,
      ...
      {
        s_log.Info(
          string.Format(WCell_RealmServer.DBCLoadFailed, "gtChanceToSpellCrit.dbc"));
        return false;
      }
      ...
      {
        s_log.Info(
          string.Format(WCell_RealmServer.DBCLoadFailed, "gtChanceToMeleeCrit.dbc"));
        return false;
      }
      ...
      {
        s_log.Info(string.Format(WCell_RealmServer.DBCLoadFailed, "gtCombatRatings.dbc"));
        return false;
      }

      if(!LoadGtClassHealthRegenPerSpiritDBC())
      {
        s_log.Info(string.Format(WCell_RealmServer.DBCLoadFailed, "GtRegenHPPerSpt.dbc"));
        return false;
      }

      if(!LoadGtClassManaRegenPerSpiritDBC())
      {
        s_log.Info(string.Format(WCell_RealmServer.DBCLoadFailed, "GtRegenMPPerSpt.dbc"));
        return false;
      }

      if(!LoadGtClassOCTHealthRegenDBC())
      {
        s_log.Info(string.Format(WCell_RealmServer.DBCLoadFailed, "GtOCTRegenHP.dbc"));
        return false;
      }

      if(!LoadGtClassOCTManaRegenDBC())
      {
        s_log.Info(string.Format(WCell_RealmServer.DBCLoadFailed, "GtOCTRegenMP.dbc"));
        return false;
      }
    ...
    {
      if(level < BaseRegen.Length)
        return BaseRegen[level];
      return BaseRegen[BaseRegen.Length - 1];
    }
  }
}