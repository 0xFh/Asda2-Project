...
namespace WCell.RealmServer.Spells.Effects
{
  ...
  public class ThreatHandler : SpellEffectHandler
  {
    ...
    protected override void Apply(WorldObject target, ref DamageAction[] actions)
    {
      ...
      if(casterUnit == null)
        return;
      if(Effect.Spell.Id == 206U)
      {
        foreach(WorldObject objectsInRadiu in casterUnit.GetObjectsInRadius(
          25f, ObjectTypes.Unit, false, int.MaxValue))
        {
          Character chr2 = objectsInRadiu as Character;
          NPC npc2 = objectsInRadiu as NPC;
          if(chr2 != null && chr2 != casterUnit && chr2.IsHostileWith(casterUnit))
            OnCharacterProvoked(chr2, casterUnit);
          else if(npc2 != null)
            npc2.ThreatCollection[casterUnit] += casterUnit.GetGeneratedThreat(
              ...
              Effect.Spell.Schools[0], Effect);
        }
      }
      else if(chr1 != null)
      {
        OnCharacterProvoked(chr1, casterUnit);
      }
      else
      {
        if(npc1 == null)
