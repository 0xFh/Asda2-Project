...
namespace WCell.RealmServer.Spells.Effects
{
  ...
  public class ApplyAuraEffectHandler : SpellEffectHandler
  {
    ...
    public override SpellFailedReason InitializeTarget(WorldObject target)
    {
      ...
      AuraEffectHandler auraEffectHandler = Effect.CreateAuraEffectHandler(m_cast.CasterReference,
        (Unit) target, ref failedReason, m_cast);
      if(failedReason == SpellFailedReason.Ok)
        m_auraEffectHandlers.Add(new SingleAuraApplicationInfo((Unit) target, auraEffectHandler));
      return failedReason;
    }
    ...
    {
      foreach(SingleAuraApplicationInfo auraEffectHandler in m_auraEffectHandlers)
      {
