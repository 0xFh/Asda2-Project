...
namespace WCell.RealmServer.Global
{
  ...
  public class ZoneSpacePartitionNode
  {
      ...
      else
      {
        for(int index1 = 0; index1 < 2; ++index1)
        {
          for(int index2 = 0; index2 < 2; ++index2)
          {
            ZoneSpacePartitionNode child = m_children[index1, index2];
            if(child.Bounds.Intersects(ref box).HasAnyFlag(IntersectionType.Touches))
              child.GetEntitiesInArea(ref box, entities, phase, ref limitCounter);
          }
        }
      }
    }
        ...
        for(int index1 = 0; index1 < 2; ++index1)
        {
          for(int index2 = 0; index2 < 2; ++index2)
          {
            ...
            if(child.Bounds.Intersects(ref sphere).HasAnyFlag(IntersectionType.Touches))
              child.GetObjectsOfSpecificType(ref sphere, entities, phase, ref limitCounter);
          }
        }
      }
    }
      ...
      else
      {
        for(int index1 = 0; index1 < 2; ++index1)
        {
          for(int index2 = 0; index2 < 2; ++index2)
          {
            ZoneSpacePartitionNode child = m_children[index1, index2];
            if(child.Bounds.Intersects(ref sphere).HasAnyFlag(IntersectionType.Touches))
              child.GetEntitiesInArea(ref sphere, entities, phase, ref limitCounter);
          }
        }
      }
    }
    ...
    {
      return GetType().Name +
             (!IsLeaf ? "" : (object) (" (" + m_objects.Count + " Objects)")) +
             m_bounds;
    }
  }
}