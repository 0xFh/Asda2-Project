...
namespace WCell.RealmServer.Items
{
  public class ItemRandomSuffixConverter : AdvancedDBCRecordConverter<ItemRandomSuffixEntry>
  {
    public override ItemRandomSuffixEntry ConvertTo(byte[] rawData, ref int id)
    {
      ...
      uint num = 5;
      int field = 0;
      entry.Id = id = GetInt32(rawData, field++);
      field += 0x12;
      entry.Enchants = new ItemEnchantmentEntry[num];
      entry.Values = new int[num];
      for(int i = 0; i < num; i++)
      {
        uint num4 = GetUInt32(rawData, field);
        if(num4 != 0)
        {
          ...
          if(enchantmentEntry != null)
          {
            ...
            entry.Values[i] = GetInt32(rawData, field + ((int) num));
          }
        }
      ...
      ArrayUtil.Trunc(ref entry.Enchants);
      ArrayUtil.TruncVals(ref entry.Values);
      return entry;
    }
  }
}