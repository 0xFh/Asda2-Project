...
namespace WCell.RealmServer.Items
{
  ...
  public class ItemSet
  {
    ...
    {
      if((long) id >= ItemMgr.Sets.Length)
        return false;
    ...
    public bool Create(Character owner)
    {
      ...
      if(freeSlot != byte.MaxValue)
      {
        ...
        if(container == null)
        {
          log.Error("Invalid container template id for ItemSet: " + Asda2ItemId.End);
        }
        else
        {
          InventoryError inventoryError =
            equippedContainers.TryAdd(freeSlot, container, true, ItemReceptionType.Receive);
          if(inventoryError == InventoryError.OK)
          {
            ...
            {
              int amount = 1;
              InventoryError error = baseInventory.TryAdd(Templates[index], ref amount,
                ItemReceptionType.Receive);
              if(error != InventoryError.OK)
              {
                ItemHandler.SendInventoryError(owner.Client, null, null,
                  error);
                log.Error("Failed to add item (Template: {0}) to bag on {1} ({2})",
                  Templates[index], owner, error);
                return false;
              }
            }
          ...
          log.Error("Failed to add ItemSet-bag to owner {0} ({1})", owner,
            inventoryError);
          return false;
        }
      }
    ...
    public class ItemSetDBCConverter : AdvancedDBCRecordConverter<ItemSet>
    {
      public override ItemSet ConvertTo(byte[] rawData, ref int id)
      {
        ...
        itemSet.Name = GetString(rawData, 1);
