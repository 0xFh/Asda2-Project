  ...
  public static class PerformanceCounters
  {
    ...
    public static void InitCounters()
    {
      if(!PerformanceCounterCategory.Exists("WCell"))
        throw new Exception(
          "WCell.PerformanceCounterInstaller.exe has not been run. Please run it and restart the application");
      PacketsSentPerSecond = new PerformanceCounter("WCell", "Packets Sent/sec", false);
      PacketsReceivedPerSecond =
        new PerformanceCounter("WCell", "Packets Received/sec", false);
      TotalBytesSent = new PerformanceCounter("WCell", "Bytes Sent", false);
      TotalBytesReceived = new PerformanceCounter("WCell", "Bytes Received", false);
      NumbersOfClientsInAuthQueue = new PerformanceCounter("WCell", "Auth Queue Size", false);
    }
  }
}