...
namespace WCell.RealmServer.Skills
{
  public static class SkillHandler
  {
    ...
    private static MappedDBCReader<SkillTiers, SkillTierConverter> TierReader;
    private static MappedDBCReader<SkillRaceClassInfo, SkillRaceClassInfoConverter> RaceClassReader;
    ...
    {
      foreach(SkillAbility[] skillAbilityArray in AbilitiesBySkill)
      {
    ...
    {
      if((long) id >= AbilitiesBySkill.Length)
        return null;
    ...
    {
      return Array.Find(GetAbilities(skill),
        ability => ability.Spell.SpellId == spell);
    }
    ...
    public class SkillLineConverter : AdvancedDBCRecordConverter<SkillLine>
    {
      public override SkillLine ConvertTo(byte[] rawData, ref int id)
      {
        ...
        int offset = 0;
        id = (int) (line.Id = (SkillId) GetUInt32(rawData, offset++));
        line.Category = (SkillCategory) GetInt32(rawData, offset++);
        line.SkillCostsDataId = GetInt32(rawData, offset++);
        line.Name = GetString(rawData, ref offset);
        GetString(rawData, ref offset);
        GetInt32(rawData, offset++);
        GetString(rawData, ref offset);
        GetInt32(rawData, offset);
    ...
    public class SkillTierConverter : AdvancedDBCRecordConverter<SkillTiers>
    {
      public override SkillTiers ConvertTo(byte[] rawData, ref int id)
      {
        ...
        int num = 0;
        id = (int) (result.Id = GetUInt32(rawData, num++));
        result.Id = (uint) id;
        uint[] array = new uint[16];
        uint[] array2 = new uint[16];
        for(int j = 0; j < 16; j++)
        {
          array[j] = GetUInt32(rawData, num + j);
          array2[j] = GetUInt32(rawData, num + j + 16);
        }

        result.MaxValues = (from i in array2
          where i != 0u
          select i).ToArray();
        result.Costs = array.Take(result.MaxValues.Length).ToArray();
        return result;
      }
    }
