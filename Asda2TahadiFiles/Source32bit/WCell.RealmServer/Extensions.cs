...
namespace WCell.RealmServer
{
  ...
  public static class Extensions
  {
    ...
    {
      return damages.Aggregate(DamageSchoolMask.None,
        (current, dmg) => current | dmg.School);
    }

    public static bool IsValid(this IWorldLocation location)
    {
      if(location.Position.X != 0.0 && location.Map != null)
    ...
    public static XmlNode Add(this XmlNode el, string name, object value, params object[] args)
    {
      ...
      XmlNode xmlNode = el.AppendChild(xmlDocument.CreateElement(name));
      xmlNode.AppendChild(xmlDocument.CreateTextNode(string.Format(value.ToString(), args)));
      return xmlNode;
    }
    ...
    public static XmlNode AddAttr(this XmlNode el, string name, object value, params object[] args)
    {
      ...
      xmlAttribute.AppendChild(xmlDocument.CreateTextNode(string.Format(value.ToString(), args)));
      return xmlAttribute;
    }
    ...
    {
      XmlDocument xmlDocument = el.OwnerDocument ?? el as XmlDocument;
      return el.AppendChild(xmlDocument.CreateElement(name)) as XmlElement;
    }
