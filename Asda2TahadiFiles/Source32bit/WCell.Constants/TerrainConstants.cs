...
namespace WCell.Constants
{
  public static class TerrainConstants
  {
    ...
    {
      return string.Format("{0:00}_{1:00}", tileX, tileY);
    }

    public static string GetMapFilename(int tileX, int tileY)
    {
      return GetTileName(tileY, tileX) + "map";
    }

    public static string GetWMOFile(int x, int y)
    {
      return string.Format("{0:00}_{1:00}.wmo", x, y);
    }

    public static string GetM2File(int x, int y)
    {
      return string.Format("{0:00}_{1:00}.m2x", x, y);
    }
    ...
    {
      tilePosition.X = (float) ((tilePosition.X - 17066.666015625) * -1.0);
      tilePosition.Y = (float) ((tilePosition.Y - 17066.666015625) * -1.0);
    }
