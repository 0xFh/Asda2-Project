...
namespace WCell.Constants
{
  public static class PositionUtil
  {
    ...
    {
      float tileFraction1 = GetTileFraction(worldPos.X);
      float tileFraction2 = GetTileFraction(worldPos.Y);
      chunkX = (int) GetChunkFraction(tileFraction1);
      chunkY = (int) GetChunkFraction(tileFraction2);
    }

    public static Rect GetTileBoundingRect(int tileX, int tileY)
    {
      float x = (float) (17066.666015625 - tileX * 533.333312988281);
      float num1 = x - 533.3333f;
      float y = (float) (17066.666015625 - tileY * 533.333312988281);
    ...
    {
      return (float) ((17066.666015625 - loc) / 533.333312988281);
    }

    private static float GetChunkFraction(float tileFraction)
    {
      return (float) ((tileFraction - (double) (int) tileFraction) * 16.0);
    }
    ...
    {
      int tileFraction1 = (int) GetTileFraction(worldPos.X);
      int tileFraction2 = (int) GetTileFraction(worldPos.Y);
      VerifyTileCoords(tileFraction1, tileFraction2);
      return new Point2D
      {
        X = tileFraction1,
        Y = tileFraction2
    ...
    public static Point2D GetXYForPos(Vector3 worldPos, out Point2D tileCoord)
    {
      ...
      int chunkFraction1 = (int) GetChunkFraction(tileFraction1);
      int chunkFraction2 = (int) GetChunkFraction(tileFraction2);
      VerifyPoint2D(worldPos, (int) tileFraction1, (int) tileFraction2, chunkFraction1,
        chunkFraction2);
      return new Point2D
      {
        X = chunkFraction1,
    ...
    public static Point2D GetHeightMapXYForPos(Vector3 worldPos, out Point2D tileCoord, out Point2D chunkCoord)
    {
      ...
      VerifyTileCoords(tileX, tileY);
      tileCoord = new Point2D { X = tileX, Y = tileY };
      float chunkFraction1 = GetChunkFraction(tileFraction1);
      float chunkFraction2 = GetChunkFraction(tileFraction2);
      ...
      int heightMapFraction1 = (int) GetHeightMapFraction(chunkFraction1);
      int heightMapFraction2 = (int) GetHeightMapFraction(chunkFraction2);
      return new Point2D
      {
      ...
      out Point2D chunkCoord, out Point2D unitCoord)
    {
      ...
      VerifyPoint2D(worldPos, tileX, tileY, chunkX, chunkY);
      chunkCoord = new Point2D { X = chunkX, Y = chunkY };
      float heightMapFraction1 = GetHeightMapFraction(chunkFraction1);
      float heightMapFraction2 = GetHeightMapFraction(chunkFraction2);
      ...
      {
        FractionX = heightMapFraction1 - heightMapX,
        FractionY = heightMapFraction2 - heightMapY
      };
    }

    private static float GetHeightMapFraction(float chunkLocFraction)
    {
      return (float) ((chunkLocFraction - (double) (int) chunkLocFraction) * 8.0);
    }
