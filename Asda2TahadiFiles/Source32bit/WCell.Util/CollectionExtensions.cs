...
namespace WCell.Util
{
  public static class CollectionExtensions
  {
    ...
    {
      List<TOutput> outputList = new List<TOutput>(enumerable.Count());
    ...
    public static TOutput[] TransformArray<TInput, TOutput>(this IEnumerable<TInput> enumerable,
      Func<TInput, TOutput> transformer)
    {
      TOutput[] outputArray = new TOutput[enumerable.Count()];
    ...
    public static string GetFullMemberName(this MemberInfo member)
    {
      ...
      if(member is MethodInfo)
        str = str + "(" +
              ((MethodBase) member).GetParameters().ToString(", ",
                param => (object) param.ParameterType.Name) + ")";
      return str;
    }
    ...
    public static Type GetActualType(this MemberInfo member)
    {
      ...
      if(type1.IsArray)
      {
        ...
        if(type2 == null)
          throw new Exception(string.Format("Unable to get Type of Array {0} ({1}).", type1,
