...
namespace WCell.Util
{
  ...
  public class EmailAddressParser
  {
    ...
    {
      inputemail = email;
      string email1 = Trim(RemoveBrackets(email));
      account = domain = "";
      if(!ParseAddress(email1) ||
         (Account.Length > 64 || Domain.Length > byte.MaxValue ||
          !DomainValid(TLDrequired)) || TLDrequired && !DomainExtensionValid())
        return;
      syntaxvalid = true;
    }
    ...
    {
      Match match = AddressRegex.Match(email);
