...
namespace WCell.Util
{
  public static class ArrayUtil
  {
    ...
    public static void Trunc<T>(ref T[] arr) where T : class
    {
      ...
      {
        if(arr[newSize] != null && newSize != num)
        {
    ...
    public static void TruncVals<T>(ref T[] arr) where T : struct
    {
      ...
      for(int newSize = 0; newSize <= num; ++newSize)
      {
        if(arr[newSize].Equals(default(T)) && newSize != num)
        {
          Array.Resize(ref arr, newSize);
          break;
        }
    ...
    {
      if(index >= arr.Length)
        EnsureSize(ref arr, (int) (index * 1.5) + 1);
      arr[index] = val;
    }
    ...
    {
      uint freeIndex = arr.GetFreeIndex();
      if(freeIndex >= arr.Length)
        EnsureSize(ref arr, (int) (freeIndex * 1.5) + 1);
      arr[freeIndex] = val;
      return freeIndex;
    }
    ...
    public static void Concat<T>(ref T[] arr, T[] values)
    {
      ...
      Array.Resize(ref arr, length + values.Length);
      Array.Copy(values, 0, arr, length, values.Length);
    }
    ...
    public static uint AddOnlyOne<T>(ref T[] arr, T val)
    {
      uint freeIndex = arr.GetFreeIndex();
      if(freeIndex >= arr.Length)
        EnsureSize(ref arr, (int) freeIndex + 1);
      arr[freeIndex] = val;
      return freeIndex;
    }
    ...
    public static uint GetFreeIndex<T>(this T[] arr)
    {
      ...
      while(num < arr.Length &&
            (arr[num] != null && !arr[num].Equals(default(T))))
        ++num;
      return num;
    }
    ...
    public static bool EqualValues<T>(this T[] arr, T[] arr2) where T : struct
    {
      ...
      {
        if(!arr[index].Equals(arr2[index]))
          return false;
      }
