  ...
  public static class WCellConstants
  {
    ...
    public static readonly Dictionary<ClassMask, ClassId> ClassTypesByMask =
      ((Func<Dictionary<ClassMask, ClassId>>) (() =>
      {
        ...
        for(int index = 0; index < AllClassMasks.Length; ++index)
        {
          if(Utility.GetSetIndices((uint) AllClassMasks[index]).Length == 1)
            dictionary.Add(AllClassMasks[index], AllClassIds[index]);
        }
    ...
    public static readonly Dictionary<RaceMask, RaceId> RaceTypesByMask =
      ((Func<Dictionary<RaceMask, RaceId>>) (() =>
      {
        ...
        for(int index = 0; index < AllRaceMasks.Length; ++index)
        {
          if(Utility.GetSetIndices((uint) AllRaceMasks[index]).Length == 1)
            dictionary.Add(AllRaceMasks[index], AllRaceIds[index]);
        }
    ...
    {
      RaceId raceId;
      if(!RaceTypesByMask.TryGetValue(mask, out raceId))
        Debugger.Break();
      return raceId;
    }
  }
